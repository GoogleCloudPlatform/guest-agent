...
Source1: %{name}-ng_%{version}.orig.tar.gz
%endif
...
%if 0%{?build_plugin_manager}
pushd %{name}-ng-%{version}/
  make cmd/google_guest_agent/google_guest_agent
popd
%endif
...
%if 0%{?build_plugin_manager}
install -p -m 0755 %{name}-ng-%{version}/cmd/google_guest_agent/google_guest_agent %{buildroot}%{_bindir}/google_guest_agent_manager
%endif
