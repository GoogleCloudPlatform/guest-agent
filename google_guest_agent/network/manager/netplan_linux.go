...
func (n *netplan) SetupEthernetInterface(ctx context.Context, config *cfg.Sections, nics *Interfaces) error {
	...
	reload1, reloadInterfaces, err := n.writeNetplanEthernetDropin(mtuMap, googleInterfaces, googleIpv6Interfaces)
	if err != nil {
	...
	if reload1 || reload2 {
		if err := n.reloadConfigs(ctx, reloadInterfaces); err != nil {
...
func (n *netplan) reloadConfigs(ctx context.Context, interfaces map[string]bool) error {
	...
	// Don't attempt to reconfigure if we have no interfaces to.
	if len(interfaces) == 0 {
		return nil
	}

	// Force reconfigure of changed interfaces and primary interface.
	cmdArgs := []string{"reconfigure"}
	for k := range interfaces {
		cmdArgs = append(cmdArgs, k)
	}

	if err := run.Quiet(ctx, "networkctl", cmdArgs...); err != nil {
		return fmt.Errorf("error reconfiguring systemd-networkd network configs: %v", err)
	}
...
func (n *netplan) writeNetplanEthernetDropin(mtuMap map[string]int, interfaces, ipv6Interfaces []string) (bool, map[string]bool, error) {
	...
	reloadInterfaces := make(map[string]bool)
	for i, iface := range interfaces {
		// We want to return primary as well hence we populate the map here even if the
		// condition of shouldManageInterface() is false.
		reloadInterfaces[iface] = true
	...
	return true, reloadInterfaces, nil
}
